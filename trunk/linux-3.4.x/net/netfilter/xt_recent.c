 ...
 */
static void recent_entry_reap(struct recent_table *t, unsigned long time,struct recent_entry *working, bool update)
{
	struct recent_entry *e;
...
recent_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (info->check_set & XT_RECENT_SET)
		...
		if (info->check_set & XT_RECENT_REAP)
			recent_entry_reap(t, time, e,info->check_set & XT_RECENT_UPDATE && ret);
	}
